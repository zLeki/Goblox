...
func GetFriends(username string) FriendData {
	ID := GetIdFromUsername(username)
	req := formatter.FormatRequest(nil, "https://friends.roblox.com/v1/users/"+strconv.Itoa(ID.PlayerID)+"/friends?userSort=Alphabetical&_=1641935441685", "GET", nil)
	resp, _ := client.Do(req)
	var data FriendData
	err := json.NewDecoder(resp.Body).Decode(&data)
	if err != nil {
...
func GetFollowers(username string, limit int) FollowerData {
	ID := GetIdFromUsername(username)
	req := formatter.FormatRequest(nil, "https://friends.roblox.com/v1/users/"+strconv.Itoa(ID.PlayerID)+"/followings?sortOrder=Asc&limit="+strconv.Itoa(limit)+"&_=1641940130441", "GET", nil)
	resp, _ := client.Do(req)
...
func AcceptFriend(username string, acc *account.Account) (error, string) {
	ID := GetIdFromUsername(username)
	req := formatter.FormatRequest(nil, "https://friends.roblox.com/v1/users/"+strconv.Itoa(ID.PlayerID)+"/accept-friend-request", "POST", nil)
...
func DeclineFriend(username string, acc *account.Account) (error, string) {
	ID := GetIdFromUsername(username)
	req := formatter.FormatRequest(nil, "https://friends.roblox.com/v1/users/"+strconv.Itoa(ID.PlayerID)+"/decline-friend-request", "POST", nil)
	req.AddCookie(acc.RobloSecurity)
	resp, _ := client.Do(req)
	switch resp.StatusCode {
	case 200:
...
func SendFriendRequest(username string, acc *account.Account) (error, string) {
	ID := GetIdFromUsername(username)
	req := formatter.FormatRequest(nil, "https://friends.roblox.com/v1/users/"+strconv.Itoa(ID.PlayerID)+"/request-friendship", "POST", nil)
	req.AddCookie(acc.RobloSecurity)
	resp, _ := client.Do(req)
	switch resp.StatusCode {
	case 200:
